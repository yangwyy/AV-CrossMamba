...
class dataset(data.Dataset):
    def __init__(self,
        ...
        # 关键修改1：为未知说话人分配一个特殊ID（值为说话人字典长度）
        self.unknown_speaker_id = len(speaker_dict) if speaker_dict else 0
    ...
    def __getitem__(self, index):
        ...
        # print("mixtures.shape:{}".format(np.asarray(mixtures).shape)) # (B(3), 64000)
        # print("audios.shape:{}".format(np.asarray(audios).shape)) # (3, 64000)
        # print("visuals.shape:{}".format(np.asarray(visuals).shape)) # (3, 100, 512)
...
if __name__ == '__main__':
    ...
    for a_mix, a_tgt, v_tgt, speakers in tqdm.tqdm(data_loader):
        # print("a_mix.squeeze().size():{}".format(a_mix.squeeze().size()))
        # print("a_tgt.squeeze().size():{}".format(a_tgt.squeeze().size()))
        # print("v_tgt.squeeze().size():{}".format(v_tgt.squeeze().size()))
        pass
